...
public class LlmAgentClient {
    ...
    public InvokeResponse execute(PrepareResponse prepared) {
        ...
        for (String model : models) {
            ...
            try {
                return doExecute(prepared, model, start);
    ...
    private InvokeResponse doExecute(PrepareResponse prepared, String model, long start) {
        ...
        messages.add(objectMapper.createObjectNode()
                .put("role", "user")
                .put("content", prepared.getRenderedPrompt()));
        body.set("messages", messages);

        String raw = litellmClient.post()
